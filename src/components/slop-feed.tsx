...
export function SlopFeed({ slops }: { slops: Slop[] }) {
  if (slops.length === 0) {
    return (
      <div className="rounded-[8px] border border-dashed border-line bg-white p-8 text-center">
        <h2 className="text-xl font-black">No slop in this window yet</h2>
        <p className="mt-2 text-muted">A fresh leaderboard is waiting to be ruined.</p>
      </div>
    );
  }
